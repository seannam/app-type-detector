...
# Print the SHA256 hex digest of a file. Wraps shasum on macOS, sha256sum on
# Linux. Returns empty output and exit 1 if the file is missing.
sha256_file() {
  local path="$1"
  [ -f "$path" ] || return 1
  if command -v shasum >/dev/null 2>&1; then
    ...
    die "neither shasum nor sha256sum is available" 11
  fi
}

# Print the version skill's self-version. Reads $VERSION_SKILL_ROOT/VERSION
# (a semver string) with a 0.0.0 fallback so legacy installs still work.
skill_version() {
  local f="$VERSION_SKILL_ROOT/VERSION"
  if [ -f "$f" ]; then
    tr -d '[:space:]' < "$f"
  else
    printf '0.0.0'
